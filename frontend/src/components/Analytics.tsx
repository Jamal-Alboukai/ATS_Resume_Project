...
type Resume = Awaited<ReturnType<typeof atsApi.getAllResumes>>[number];
...
export function Analytics() {
  ...
  const loadAnalyticsData = async (): Promise<void> => {
    try {
      ...
      setExportIds(completedResumes.map(r => r.id));
    } catch (error) {
  ...
  const handleExport = async () => {
    if (!exportIds.length) {
      setExportError('No completed analyses available to export.');
      return;
    }
    setExportError(null);
    setIsExporting(true);
    try {
      const blob = await atsApi.exportAnalysis(exportIds, exportFormat);
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `ats-analytics.${exportFormat}`;
  ...
  const generateOverviewData = (resumes: Resume[], completedResumes: Resume[]): OverviewStats => {
    const totalApplications = resumes.length;
    const avgScore = completedResumes.length
      ? completedResumes.reduce((sum, r) => sum + (r.analysis?.overallScore || 0), 0) / completedResumes.length
      : 0;
    ...
    const conversionRate = totalApplications
      ? Number(((excellentCandidates / totalApplications) * 100).toFixed(1))
      : 0;
  ...
  const generateSkillTrends = (resumes: Resume[]): SkillTrend[] => {
  ...
  const generateScoreTrends = (_resumes: Resume[]): ScoreTrendPoint[] => {
  ...
  const generateApplicationVolume = (_resumes: Resume[]): ApplicationVolumePoint[] => {
  ...
  const generateTopSkills = (_resumes: Resume[]): TopSkill[] => {
  ...
  const generatePerformanceMetrics = (_resumes: Resume[]): PerformanceMetric[] => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="rounded-2xl p-6 shadow-xl" style={glassStyle}>
          ...
          <h3 className="text-2xl text-gray-800">{analyticsData.overview.rejectedCount}</h3>
