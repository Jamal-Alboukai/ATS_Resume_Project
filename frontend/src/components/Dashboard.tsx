...
type ResumeItem = {
  ...
  filename: string;
  uploadDate: string;
  status: 'completed' | 'processing' | 'failed' | 'queued' | 'rejected' | string;
  analysis?: { overallScore: number; skills?: { name: string }[] };
};

type DashboardData = {
  monthlyApplications: { month: string; applications: number }[];
  skillsDistribution: { name: string; value: number; color: string }[];
  processingQueue: { name: string; progress: number; id: string }[];
...
export function Dashboard() {
  ...
  const generateMonthlyData = (resumes: ResumeItem[]) => {
  ...
  const generateSkillsDistribution = (resumes: ResumeItem[]) => {
  ...
  const getStatusColor = (status: ResumeItem['status']) => {
    switch (status) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105" style={glassStyle}>
          ...
          <div className="mt-4">
            <h3 className="text-2xl text-gray-800">{stats.rejected}</h3>
            <p className="text-sm text-gray-600 mt-1">Rejected Resumes</p>
          </div>
        </div>
      </div>
